...
`reconstituteAggregateFromHistory(createReplayTarget, events)` builds the replay target through your factory and replays the events into it. It yields the aggregate only in the `Ok`, so a rejected replay leaves you with nothing to return by mistake. The replay advances the version and leaves `pendingEvents` empty. Replayed events are historical facts, not new facts. Every load from an `EventStore` goes through `readStreamPages` and `reconstituteAggregateFromStreamPages`, whatever the stream length. `reconstituteAggregateFromHistory` serves a history that is already in memory. See [Event Sourcing -> Loading from history](./event-sourcing.md#loading-from-history).
