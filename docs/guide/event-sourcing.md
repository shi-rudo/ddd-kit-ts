...
`reconstitute` factory does, so a stored snapshot state is a fact like the
history and a rule that tightened later does not break every restore. The
section [Where Invariants Live](./aggregates.md#where-invariants-live)
