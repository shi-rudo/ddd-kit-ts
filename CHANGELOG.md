...
`deepFreeze`'s Date/Map/Set mutator shadows are now shared module-level functions instead of per-instance closures — `createDomainEvent` freezes a Date per event, so the previous 16 closure allocations per event were pure churn on the hot path. The mutator blocking is documented as deny-by-enumeration (a future runtime mutator like the stage-3 `Map.prototype.getOrInsert` is not blocked until the list is updated — guard rail, not security boundary). The clone-vs-alias rule between `deepEqual` and `deepOmit` now lives in one exported set (`REFERENCE_COMPARED_TAGS` in `is-built-in.ts`), so the two modules cannot drift; `Entity`'s class-instance state ownership-transfer contract is documented on the public constructor and `setState`.
