...
service needs: an outbox with a dispatcher, command idempotency,
projections, snapshots, durable deadlines, and the `@shirudo/ddd-kit/money`
entry point. Every port ships an adapter contract suite.

The [v3 migration guide](docs/guide/migrating-to-v3.md) gives a before and
after for every breaking change below, with the 3.0.0 names. Its appendices
lead a user of a 3.0.0 release candidate to 3.0.0. The release candidates
are not source-compatible with each other, and every rc tag stays
immutable. Run one kit version per process during a cutover.

### Changed (breaking)

#### Persistence and the Unit of Work

- The Unit of Work owns the write tracking of an aggregate. `IRepository`
  and `IUnitOfWorkRepository` are gone. A port extends
  `AggregatePersistence<TAggregate, TId>` or `Repository<TAggregate, TId>`,
  which adds `remove` for a physical removal. `add` and `update` register
  the write intent synchronously, and the flush writes at commit, in the
  transaction of the outbox. `save` and `exists` are gone. See
  [Migrate in this order](docs/guide/migrating-to-v3.md#migrate-in-this-order).
- Loads follow the get/find contract. `findById` returns `undefined` for an
  absent aggregate, and `getById` throws `AggregateNotFoundError`. In 2.2.0,
  `getById` returned `null` and `getByIdOrFail` threw. The compiler does not
  flag a missed old `getById` call; the guide gives the three-step rename.
- `remove(aggregate)` replaces `delete(id)`. It exists only on a definition
  with `physicalRemoval: true`. A business deletion stays aggregate behavior
  followed by `update`.
...
- An adapter-owned `PersistenceModel` captures the baseline and derives the
  change set. The aggregate loses `persistedVersion`, `hasChanges`,
  `changedKeys`, `markPersisted`, `clearPendingEvents`, and the protected
  `onPersisted` hook. The `onPersisted` and `onPersistError` observers of
  `UnitOfWorkDeps` and `WithCommitDeps` run after the commit. See
  [The persistence lifecycle leaves the aggregate](docs/guide/migrating-to-v3.md#the-persistence-lifecycle-leaves-the-aggregate).
...
- Snapshots move out of the aggregate into an adapter-owned
  ...
  `TSnapshotState` type parameter are gone.
- `withCommit` takes commit tokens from its enrollment capability in place
  of the touched aggregates, and the `deleted` array is gone. See
  [`withCommit` takes commit tokens](docs/guide/migrating-to-v3.md#withcommit-takes-commit-tokens).
- The `outbox` dependency of `withCommit` and `UnitOfWork` has the type
  `OutboxWriter`.
- `IQueryableRepository` is gone. A query is a method of the consumer's own
  port, with a uniqueness law or a limit, a stable order, and a cursor. See
  [`IQueryableRepository` is gone](docs/guide/migrating-to-v3.md#iqueryablerepository-is-gone).
- `IdentityMap.set(type, aggregate)` reads the id from the aggregate
  identity; the separate id argument is gone.

#### Aggregates, entities, and events

- One lifecycle vocabulary. `AggregateRoot` is `StateStoredAggregate`,
  `IAggregateRoot` is `Aggregate`, and `IEventSourcedAggregate` is
  `ReplayableAggregate`. `commit(state, events)` is `setState(state, events)`,
  `loadFromHistory` is `replayHistory`, and `markRestored` is
  `markReconstituted`. The `handlers` of an event-sourced aggregate are
  `folds`, and a missing fold throws `MissingFoldError`. `DomainEvent.version`
  is `schemaVersion`. Both aggregate classes take `<TState, TId, TEvent>`.
  See
  [One lifecycle vocabulary](docs/guide/migrating-to-v3.md#one-lifecycle-vocabulary).
- `setState(state)` always bumps the version.
  `setStateWithoutVersionBump(state)` is the named exception, and the
  `autoVersionBump` option is gone. `apply(event)` loses its `isNew` flag. An
  event-sourced aggregate that calls `setState` throws
  `DirectStateMutationError`. The compiler does not flag a one-argument
  `setState` that now bumps; the guide says how to audit the call sites.
- The aggregate decides, and the application shell records. A domain method
  creates an uncommitted event with `createEvent`, and `recordPendingEvents`
  stamps the event id, the time, and the metadata before the registration.
  `recordEvent` is gone, and `withCommit` rejects an unrecorded event. Event
  ids and the clock come from an immutable `createDomainEventFactory` value;
  the six module setters and scoped helpers are gone. See
  [The aggregate decides; the shell records](docs/guide/migrating-to-v3.md#the-aggregate-decides-the-shell-records).
- Events are readonly and minted. A recording path accepts only an event
  that a kit constructor minted and throws `UnmintedEventError` for a hand-built
  object. `createDomainEvent` rejects binary buffers. `EventMetadata` fields
  are readonly. `DomainEvent` loses `aggregateVersion`; the commit envelope
  carries the position.
- `Entity.state` is protected, and `IEntity<TId, TState>` is `IEntity<TId>`.
  A subclass no longer sees `_state` and `freezeState`. A reconstitution
  factory passes `trustInitialState: true` for a persisted state.
- The state validator is a function in `EntityConfig.validateState`, not an
  overridable method. A subclass member named `validateState` fails to
  compile. An event-sourced aggregate runs the validator on each `apply()`.
- Replay trusts history: it runs no `validateEvent`. Both paths check the
  aggregate identity of an event. `apply()`, `setState`, and
  `addDomainEvent` stamp missing fields and throw `MisattributedEventError`
  for an event of another aggregate. Replay throws `ForeignEventError`. A fold
  that returns `undefined` throws `FoldReturnedNoStateError`, and an own
  `"__proto__"` key in a fold result or a payload throws
  `HostileStateKeyError`. See
  [Replay trusts history, and events name their aggregate](docs/guide/migrating-to-v3.md#replay-trusts-history-and-events-name-their-aggregate).
- The `Aggregate` interface has `aggregateIdentity`, the aggregate type and
  the id as one frozen `AggregateIdentity` value. A hand-written
  implementation adds it. An instance that the kit did not construct is
  rejected with `UnmanagedInstanceError`.
- `updateEntityById`, `replaceEntityById`, and `removeEntityById` return
  `ReadonlyArray<T>`, and they return the input array when nothing changed.
  `deepFreeze` takes one argument. The `Entity` constructor rejects a blank
  id with `MissingEntityIdError`, and a version that is not a safe integer of
  at least zero throws `InvalidVersionError`.
- `voEquals` and `deepEqual` compare the class of a value object instance.
  Two instances of different classes with the same props are not equal.

#### Event store

- `EventStore.append` and `readStream` take an `AggregateIdentity` in place
  of the raw id. The same id under two aggregate types is two streams. A
  production schema keys a stream on `(aggregate_type, aggregate_id,
  position)`. See
  [Streams take the aggregate identity](docs/guide/migrating-to-v3.md#streams-take-the-aggregate-identity).
- `readStream` returns a `StreamReadResult`, which reports whether the stream
  exists and the real head. Its options object and `limit` are required. An
  invalid option rejects with `RangeError` before the store is read.

#### Errors

- Every kit error is a `StructuredError` of `@shirudo/base-error` with a
  stable `code`, a `category`, and `retryable`. `error.name === error.code`,
  so the class name is no longer in `error.name`. `KitErrorCode` lists every
  code the kit produces. See
  [Match an error on its code](docs/guide/migrating-to-v3.md#match-an-error-on-its-code).
- A subclass of `DomainError` or `InfrastructureError` passes `code` and
  `message` in an options object.
- Every kit error that names one aggregate carries one `identity` field of
  the shape `{ aggregateType, aggregateId }`, and every kit error takes an
  options object. Every kit message renders an aggregate as `Type(id)`.
- `ConcurrencyConflictError` requires a `reason`: `stale_version`,
  `version_unchanged`, `aggregate_absent`, or `version_unknown`.
  `actualVersion` is `number | null`, and the `-1` sentinel is gone.
  `version_unchanged` is not retryable.
- `toProblemDetails` delegates to `toProblem` and returns
  `{ status, headers, body, outcome }`. The issues are under
  `details.issues`. The `member` option and `ValidationProblemMember` are
  gone.
- `toPublicErrorView` returns the `LocalizedPublicError` of
  `@shirudo/base-error`, and its `locale` option is a preference. The view
  carries the locale that resolved.

#### Messaging ports

- `EventBus` has `subscribeAll`, `subscribeMany`, and `close` as required
  members. `publish` accepts `PublishOptions` with a `signal` and a
  `timeoutMs` that defaults to 30 seconds. An `EventHandler` receives an
  `ExecutionContext` as its second argument. After `close()`, the bus throws
  `EventBusClosedError`.
- `OutboxWriter.add` takes `EventCommitCandidate` values. An `OutboxRecord`
  is a `CommittedDomainEvent` with a `dispatchId`: `event`, the `source`
...
- `execute` throws `UnregisteredHandlerError` for a message type without a
  handler, instead of returning it through the error channel.
- The total `errorMapper` option is gone. `mapExpectedError` classifies only
  the failures it names; every other throw propagates unchanged. A mapper
  that fails throws `ErrorMapperFailedError`.
- With a concrete message map, the map entry is the result type of
  `execute` and `executeUnsafe`.

#### Entry points and platform

- The `@shirudo/ddd-kit/utils` entry point is gone; its functions and types
  come from the package root.
- `@shirudo/ddd-kit/presentation` is `@shirudo/ddd-kit/public-errors`.
- The `deepOmit` types `Key` and `PathSegment` are `DeepOmitKey` and
  `DeepOmitPathSegment`. `computeBackoffDelay` is no longer exported.
- `engines.node` is `>=22`. Node 20 reached its end of life on 2026-04-30.
- The peer `@shirudo/base-error` is `^8.3.0`. Before 8.1.0, `redactAllow`
  does not mask the fields that a kit error adds to its log object.
- The repository contract suites are the v3 suites under the same names.
  Their failure messages start with "Contract violated:" and "Contract test
  skipped:".
...
- `versionedFlush` builds a flush from the store statements. It owns the
  branches of the flush contract: a unique violation on `add` becomes
  `DuplicateAggregateError`, and a compare-and-set that matches no row
  becomes `ConcurrencyConflictError` with its reason. `VersionedWrite` and
...
- `SnapshotModel` with `defineSnapshotModel`, `captureAggregateSnapshot`, and
  ...
  captured snapshot is deep-frozen.
- `readStreamPages` reads a stream in bounded pages toward the head that it
  pins on the first page. `reconstituteAggregateFromStreamPages` replays the
  pages and checks that the replay ends at that head. `pinTargetVersion`
  gives the window decision as a pure function. A point-in-time read passes
  `toVersion`. `InvalidEventStreamPageError`, `ReplayTargetMismatchError`,
  and `ReplayRejectedError` name the failures.
- Contract suites in `@shirudo/ddd-kit/testing`:
  `createRepositoryContractTests`, `createEsRepositoryContractTests`,
  `createEventStoreContractTests`, `createSnapshotStoreContractTests`, and
  `createReplayableStreamPagesContractTests`, with
  `createReplayableStreamPages` for a replay test without a store.
- `Specification<T>` and `specification(name, predicate)` name executable
...
- `createDomainEventFactory({ eventIdFactory, clock, source })` and the frozen
  `defaultDomainEventFactory`. `recordPendingEvents` accepts shared stamp
  options for a batch. `createDomainEventFromFacts` builds an event whose
  identity and time the caller owns.
- `AggregateConfig.maxPendingEvents` limits the pending list with
  `PendingEventLimitExceededError`. `DuplicateEventIdError` and
  `ReentrantEventRecordingError` protect the recording.
- `EntityConfig.trustInitialState`, `detachState`, `toVersion`, and
  `reconstituteAggregateFromHistory`.
- `DomainEventValidationError` and `SnapshotTimeValidationError` carry a
  `code` and a `field`.
- `isDomainErrorLike`, `isInfrastructureErrorLike`, and `isWiringErrorLike`
  recognize an error from another copy of the kit. The capability registries
  recognize an aggregate from another copy and fail loud with
  `CapabilityRegistryConflictError` on a foreign value.

#### Messaging and delivery

- `OutboxDispatcher` delivers the outbox to an `OutboxSink` at least once,
  with bounded retries, dead letters, required observers, and a shared
  `classifyFailure` policy. `eventBusSink` delivers to the in-process bus.
  `outboxWriterAcceptingEventLoss()` names a setup without delivery
...
- `IntegrationMessage` is the JSON-safe broker contract, with
  ...
  `correlationId`, `conversationId`, and `causationId` are headers of the
  envelope.
- A process manager routes commands through a transactional command outbox:
  `PublishedCommand`, `CommandOutboxWriter`, `routeEventsToCommandOutbox`,
  and `createCommandOutboxContractTests`.
- `EventBusImpl` reports handler failures, aborted publications, and
  accumulating subscriptions through optional observers. It bounds one
  publish chain with `maxPublishDepth` and `PublishDepthExceededError`, and an
  optional `chainStore` follows a chain across async work.
  `createEventBusContractTests` proves an implementation of the port.
- `ExecutionContext` carries a `signal` and a `deadlineAt` to delivery
  handlers, and every delivery has a default bound of 30 seconds.

#### Application

- Command idempotency: the `IdempotencyStore` port, `withIdempotentCommit`,
  `InMemoryIdempotencyStore`, leased claims with reconciliation, and
  `createIdempotencyStoreContractTests`.
- Projections: the `ProjectionCheckpointStore` port, the `Projector` runner,
  `InMemoryProjectionCheckpointStore`, `projectionFromHandlers`, and
  `createProjectionCheckpointStoreContractTests`. The projector rejects a gap
  or a changed receipt before it applies an event.
- Durable deadlines: the `DeadlineStore` port, `DeadlineProcessor`,
  `InMemoryDeadlineStore`, and `createDeadlineStoreContractTests`.
- `domainErrorToResult` turns the listed `DomainError` classes into `Err` and
  rethrows everything else.
- `DuplicateHandlerRegistrationError` names a second handler for one message
  type on a bus.
- A state-stored and an event-sourced checkout process manager in
  `examples/saga`.

#### Money

- The `@shirudo/ddd-kit/money` entry point. `Money` is exact, frozen plain
  data with a bigint amount in minor units, a currency, and a scale. The
  operations are exact only: add, subtract, negate, and lossless rescale. The
  kit ships no rounding. `parseMoneyInput` accepts a decimal string exactly or
  rejects it. `createMoneyFactory` binds a currency scale resolver that the
  consumer supplies. `MoneyDto` and the snapshot bridge cross JSON and
  calculation libraries. The `try*` parsers return a `Result`.

#### Public errors

- `createKitPublicErrors()` builds a public error catalog of the kit codes,
  which the consumer extends at the composition root. `toPublicErrorView`
  accepts a `catalog` option.

#### Documentation and tooling

- New guides: command idempotency, deadlines, money, sagas, and the v3
  migration guide.
- CI runs the package in Cloudflare `workerd` and in the Vercel Edge Runtime,
  and it builds the documentation.
- Mutation testing for the aggregate, the event store, and the money module,
  and property-based tests for the money laws.

### Removed

- The hand-curated `LLM.md`. The documentation build derives `llms.txt` and
  `llms-full.txt` from the guides. The package never contained the file.
...
- A value object accepts a nested value object again. Version 2.2.0 rejected
  it as a class instance.
- `deepFreeze` keeps a global or sticky `RegExp` usable. Before, a match on a
  frozen pattern threw.
- `deepOmit` keeps non-enumerable own string properties, and its
  `ignoreKeyPredicate` receives a stable path. `deepEqual` compares an opaque
...
- `analyzeDomainMachineDefinition` honors a prepared definition. The domain
  state machine validates its defensive copy, and the outcomes of
  `transitionDomainState` are frozen.
- The `Entity` constructor and `setState` copy, freeze, and validate in the
  same order. An array state keeps its non-index keys.
- A state-stored aggregate checks the aggregate identity of an event. Before,
  it recorded an event of another aggregate, and `withCommit` published it.
- A replay routes a `DomainError` from another copy of the kit into the
  `Result`.
- `InMemoryEventStore.append` costs the size of the batch, not of the stream.
  `InMemoryOutbox.getPending(limit)` stops at the limit.
