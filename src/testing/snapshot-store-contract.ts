	...
	return [
		{
			...
			run: inEnv(async (env) => {
				assertEqual(
					await env.store.load(identity("Order", "o-1")),
					undefined,
		...
		},
		{
			...
			run: inEnv(async (env) => {
				...
				await env.store.save(identity("Order", "o-1"), stored);
				const loaded = await env.store.load(identity("Order", "o-1"));
		...
		},
		{
			...
			run: inEnv(async (env) => {
				...
				const [orderX1, invoiceX1, orderX2] = await Promise.all([
					env.store.load(identity("Order", "x-1")),
					env.store.load(identity("Invoice", "x-1")),
					env.store.load(identity("Order", "x-2")),
				]);
				assert(
					orderX1?.version === 1 &&
						invoiceX1?.version === 2 &&
		...
		},
		{
			...
			run: inEnv(async (env) => {
				...
				await env.store.delete(identity("Order", "o-1"));
				await env.store.delete(identity("Order", "never-saved"));
				assertEqual(
					await env.store.load(identity("Order", "o-1")),
					undefined,
				...
				assertEqual(
					(await env.store.load(identity("Order", "o-2")))?.version,
					2,
		...
		},
		{
			...
			run: inEnv(async (env) => {
				...
				await env.store.save(identity("Order", "o-1"), input);
				...
				const reloaded = await env.store.load(identity("Order", "o-1"));
				assertEqual(
