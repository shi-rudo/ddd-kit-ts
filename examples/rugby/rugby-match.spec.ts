...
describe("RugbyMatch Aggregate", () => {
	...
	it("should be scheduled", () => {
		...
		expect(match.view.status).toBe("scheduled");
	...
	it("should score a try and update the score", () => {
		...
		expect(match.view.homeScore).toBe(5);
		expect(match.view.scoringPlays[0]?.playerName).toBe("Richie McCaw");
		expect(match.view.status).toBe("in-progress");
	});
	...
	it("should score a conversion", () => {
		...
		expect(match.view.homeScore).toBe(7);
		expect(match.view.scoringPlays[1]?.playerName).toBe("Dan Carter");
	});

	it("should score a penalty goal", () => {
		const match = RugbyMatch.schedule(matchId, homeTeam, awayTeam, new Date());
		match.scorePenaltyGoal(awayTeam.id, "Frans Steyn");
		expect(match.view.awayScore).toBe(3);
		expect(match.view.scoringPlays[0]?.playerName).toBe("Frans Steyn");
	});

	it("should be able to finish", () => {
		const match = RugbyMatch.schedule(matchId, homeTeam, awayTeam, new Date());
		match.finish();
		expect(match.view.status).toBe("finished");
	});
});
